...
def wishlist(session_id=None, course_id=None, course_num=None):
    ...
    if check_group(User.prof, session_id):
        ...
        if request.method == 'POST':
            ...
            fname = request.form['fname']
            lname = request.form['lname']
            ta_id = find_ta_id_by_name(fname,lname)
            if ta_id is None:
                return render_template('wishlist.html', session_id=session_id, course_id=course_id,
                                       course_num=course_num, msg=f"No TA Available",course_term=course_term)

            add_to_wishlist(course_id, course_term, prof_id, ta_id)
            return render_template('wishlist.html', session_id=session_id, course_id=course_id, course_num=course_num, msg=f"Added {fname} {lname}",course_term=course_term)

        return render_template('wishlist.html', session_id=session_id,course_id=course_id, course_num=course_num,course_term=course_term)
